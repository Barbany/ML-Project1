...
%\author{
%  Oriol Barbany, Natalia Gullón and Sophia Kypraiou \\
...
\author{\IEEEauthorblockN{Oriol Barbany\IEEEauthorrefmark{1},
	...
	\IEEEauthorblockA{Machine Learning (CS-433), School of Computer and Communication Sciences \\ 
		...
		\IEEEauthorrefmark{3}\href{mailto:sophia.kypraiou@epfl.ch}{sophia.kypraiou@epfl.ch}}}
...
The Higgs boson is an elementary particle in the Standard Model of physics that explains why other particles have mass. In order to discover this particle, protons are smashed one into another at high speeds to generate en smaller particles as by-products of collisions. However, the Higgs boson decays rapidly into other particles so it is difficult to observe it directly. That is why its decay signature or other products that results from its decay process are measured instead. Since many decay signatures look similar, the likelihood that a given event's signature was the result of a Higgs boson or some other process/particle needs to be estimate.

Therefore, in this work we will predict whether an event was a Higgs boson or something else from a given vector of features representing the decay signature of a collision event. This data is obtained from experiments done at the Large Hadron Collider at CERN. Thus, the aim of this project is to build a binary classifier to predict the Higgs boson particles. 
